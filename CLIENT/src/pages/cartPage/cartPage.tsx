...
// import { clearCart, addToCart, loadCartState } from '../../slices/cart/cartSlice'; // Подставьте корректный путь к вашему экшену addToCart
...
function CartPage() {
  ...
  const cartItems = selectedItems.map((item: CartItem, index: number) => (
    <li key={index} className={style["cart-item"]}>
  ...
  //   <li className={style["cart-item"]} key={item.itemId}>
  //     <li className={style["cart-item"]} key={item.itemId}>
  //       <span className={style["item-name"]}>{item.name}</span>
  //       <span className={style["item-price"]}>${item.price}</span>
  //     </li>
